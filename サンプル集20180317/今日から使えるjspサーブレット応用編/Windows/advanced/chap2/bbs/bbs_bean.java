...
public class bbs_bean implements Serializable {
	...
	public ResultSet search(String keywd){
		...
		try {
			...
			Connection db=DriverManager.getConnection(objRB.getString("conStr"));
			Statement objSql=db.createStatement();
			rs=objSql.executeQuery("SELECT id FROM bbs_master WHERE (title Like '%" + keywd + "%' OR article Like '%" + keywd + "%') AND title <> 'çÌèúçœÇ›' ORDER BY id DESC");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return rs;
	...
	public boolean removeArticle(String id, String passwd){
		...
		try {
			...
			if(rs.next()){
				objSql.executeUpdate("UPDATE bbs_master SET title='çÌèúçœÇ›' WHERE id=" + id);
			}else{
				flag=false;
			}
		} catch (Exception e) {
	...
	public String getSdat()  {
		return EncodeUtil.formatDate("yyyyîNMMåéddì˙",result.sdat1) + EncodeUtil.formatDate(" HHéûmmï™ssïb",result.sdat2);
	}
